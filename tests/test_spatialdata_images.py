...
def _image(width: int = WIDTH, height: int = HEIGHT, channels: int = 2) -> xr.DataArray:
    ...
    return xr.DataArray(
        stack, dims=("c", "y", "x"), coords={"c": [f"ch{i}" for i in range(channels)]}
    )
...
def test_explicit_window_is_recorded_and_used(tmp_path: Path) -> None:
    m = write_webp_pyramid(
        _image(), tmp_path / "w", tile_size=TILE, display_min=100, display_max=900
    )
...
def test_multiscale_input_uses_full_resolution(tmp_path: Path) -> None:
    ...
    tree = DataTree.from_dict(
        {"scale0": full.to_dataset(name="image"), "scale1": half.to_dataset(name="image")}
    )
